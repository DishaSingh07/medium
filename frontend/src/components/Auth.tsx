...
export const Auth = ({ type }: { type: "signup" | "signin" }) => {
    ...
    async function sendRequest() {
        try {
            const endpoint = `${BACKEND_URL}/api/v1/user/${type}`;
        ...
        <div className="h-screen flex justify-center flex-col">
            <div className="flex justify-center">
                <div>
                    ...
                    <div className="space-y-4">
                        {type === "signup" && <LabelledInput id="name" label="Name" placeholder="name" onChange={(e) => setPostInputs({ ...postInputs, name: e.target.value })} />}
                        <LabelledInput id="email" label="Email" placeholder="email" onChange={(e) => setPostInputs({ ...postInputs, username: e.target.value })} />
                        <LabelledInput id="password" label="Password" type="password" placeholder="password" onChange={(e) => setPostInputs({ ...postInputs, password: e.target.value })} />
...
function LabelledInput({ id, label, placeholder, onChange, type }: LabelledInputType) {
    return (
        <div>
            <label className="block mb-2 text-sm text-black font-semibold pt-4" htmlFor={id}>{label}</label>
            <input id={id} onChange={onChange} type={type || "text"} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder={placeholder} required />
        </div>
    );
}
